...
function startPomodoro() {
  ...
  studyMinutes = document.querySelector('#study-input').value;
  restMinutes = document.querySelector('#rest-input').value;

  if (!studyMinutes) studyMinutes = 25;
  if (!restMinutes) restMinutes = 5;
  ...
  setInterval(() => {
    ...
    checkPomodoroPeriod(startTime, studyMinutes, restMinutes);
  }, 1000);
}
...
function checkPomodoroPeriod(startTime, studyMinutes, restMinutes){

  studyMinutes = Math.floor(studyMinutes);
  restMinutes = Math.floor(restMinutes);

  let now = Date.now();
  if(((now - startTime)/1000/60) % (studyMinutes + restMinutes) < studyMinutes) { 
